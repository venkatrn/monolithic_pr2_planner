...
const double kCircumscribedRadius = 0.7;
...
void DistanceTransform2D::update2DHeuristicMap(const std::vector<unsigned char>& data){
    loadMap(data);
}
...
void DistanceTransform2D::loadMap(const std::vector<unsigned char>& data){
    ...
    m_gridsearch->search(m_grid, costmap_2d::LETHAL_OBSTACLE, m_obstacle_coordinates[0].first, m_obstacle_coordinates[0].second,
        0,0, SBPL_2DGRIDSEARCH_TERM_CONDITION_ALLCELLS, m_obstacle_coordinates);
    ...
    for (size_t ii = 0; ii < m_size_col; ++ii) {
      for (size_t jj = 0; jj < m_size_row; ++jj) {
        ...
        if (value < INFINITECOST) {
          dist_transform.at<double>(ii, jj) = std::min(static_cast<double>(value) * 0.001, kCircumscribedRadius);
        } else {
    ...
    cv::Mat colored_mat;
    cv::normalize(1.0 - dist_transform, colored_mat, 0, 255, cv::NORM_MINMAX, CV_8UC1);
...
double DistanceTransform2D::getIncomingEdgeProbability(GraphStatePtr state) const{
    DiscObjectState obj_state = state->getObjectStateRelMapFromState();
    ...
    const double prob = std::min(1.0, (static_cast<double>(cost) * 0.001) / kCircumscribedRadius);
    return prob;
}
...
void DistanceTransform2D::getBresenhamCirclePoints(int x0, int y0, int radius, std::vector<int>&
    ret_x,
...
void DistanceTransform2D::getBresenhamCirclePoints(int x0, int y0, int radius, std::vector<std::pair<int,int> >&
    points){
    points.clear();
...
void DistanceTransform2D::getBresenhamLinePoints(int x1, int y1, int x2, int y2, std::vector<std::pair<int, int> >&
    points){
...
void DistanceTransform2D::getBresenhamLinePoints(int x1, int y1, int x2, int y2, std::vector<int>& pts_x, std::vector<int>&
    pts_y){
    pts_x.clear();
    pts_y.clear();
